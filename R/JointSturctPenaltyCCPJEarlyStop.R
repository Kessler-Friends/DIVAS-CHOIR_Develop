ccpSubOptJPEarlyStop <- function(v0, Qo1, Qo2, Qc1, Qc2, Vo, tau) {
  ...
  psd_wrap <- getFromNamespace("psd_wrap", "CVXR")
  objective <- CVXR::quad_form(v, psd_wrap(Qo1)) - 2 * CVXR::t(v0) %*% Qo2 %*% v + CVXR::quad_form(v0, Qo2) + tau * sum(slack)
  constraints <- list()

  # constraints for each Qc1 and Qc2
  for (ic in 1:nc) {
    constraints <- c(constraints,
                     CVXR::quad_form(v, psd_wrap(Qc1[[ic]])) - 2 * CVXR::t(v0) %*% Qc2[[ic]] %*% v + CVXR::quad_form(v0, Qc2[[ic]]) <= slack[ic])
  }
  # Addition
  constraints <- c(constraints,
